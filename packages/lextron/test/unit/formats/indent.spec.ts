...
describe('Indent', () => {
  test('+1', () => {
    ...
    expect(editor.scroll.domNode).toEqualHTML(`
      <ol>
        <li class="lxt-indent-1" data-list="bullet">0123</li>
      </ol>
    `);
  });
  ...
  test('-1', () => {
    const editor = new Editor(
      createScroll(
        '<ol><li data-list="bullet" class="lxt-indent-1">0123</li></ol>',
      ),
    );
    editor.formatText(4, 1, { indent: '-1' });
    expect(editor.getChangeSet()).toEqual(
  ...
  test('1', () => {
    ...
    expect(editor.scroll.domNode).toEqualHTML(`<p class="lxt-indent-1">abc</p>`);
  });
});
