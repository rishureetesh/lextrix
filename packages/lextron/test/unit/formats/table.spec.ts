...
const tableDelta = new ChangeSet()
  .insert('A1')
  .insert('\n', { table: 'a' })
  .insert('A2')
  .insert('\n', { table: 'a' })
...
describe('Table', () => {
  test('initialize', () => {
    ...
    expect(editor.getChangeSet()).toEqual(tableDelta);
    expect(editor.scroll.domNode).toEqualHTML(tableHTML);
  });

  test('add', () => {
    const editor = new Editor(createScroll(''));
    editor.applyChangeSet(new ChangeSet([...tableDelta.ops]).delete(1));
    expect(editor.scroll.domNode).toEqualHTML(tableHTML);
  });
  ...
  test('unbalanced table applyChangeSet', () => {
    const editor = new Editor(createScroll('<p><br /></p>'));
    editor.applyChangeSet(
      new ChangeSet()
        .insert('A1\nB1\nC1\n', { table: '1' })
  ...
  test('existing table applyChangeSet', () => {
    const editor = new Editor(
      createScroll(
        `
      <table>
