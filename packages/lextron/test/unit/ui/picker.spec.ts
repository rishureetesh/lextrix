...
describe('Picker', () => {
  const setup = () => {
    ...
    const pickerSelector = container.querySelector('.lxt-picker') as HTMLElement;
    return { container, pickerSelectorInstance, pickerSelector };
  };

  test('initialization', () => {
    const { container } = setup();
    expect(container.querySelector('.lxt-picker')).toBeTruthy();
    expect(container.querySelector('.lxt-active')).toBeFalsy();
    expect(
      container.querySelector('.lxt-picker-item.lxt-selected')?.outerHTML,
    ).toEqualHTML(
      '<span tabindex="0" role="button" class="lxt-picker-item lxt-selected" data-label="0"></span>',
    );
    expect(
      container.querySelector('.lxt-picker-item:not(.lxt-selected)')?.outerHTML,
    ).toEqualHTML(
      '<span tabindex="0" role="button" class="lxt-picker-item" data-value="1" data-label="1"></span>',
    );
  });
  ...
  test('label is initialized with the correct aria attributes', () => {
    ...
    expect(
      pickerSelector
        .querySelector('.lxt-picker-label')
        ?.getAttribute('aria-expanded'),
    ).toEqual('false');
    const optionsId = pickerSelector.querySelector('.lxt-picker-options')?.id;
    expect(
      pickerSelector
        .querySelector('.lxt-picker-label')
  ...
  test('options container is initialized with the correct aria attributes', () => {
    ...
    expect(
      pickerSelector
        .querySelector('.lxt-picker-options')
    ...
    const ariaControlsLabel = pickerSelector
      .querySelector('.lxt-picker-label')
      ?.getAttribute('aria-controls');
    expect(pickerSelector.querySelector('.lxt-picker-options')?.id).toEqual(
      ariaControlsLabel,
    );
    expect(
      (pickerSelector.querySelector('.lxt-picker-options') as HTMLSelectElement)
        .tabIndex,
    ).toEqual(-1);
  });
  ...
  test('aria attributes toggle correctly when the picker is opened via enter key', () => {
    const { pickerSelector } = setup();
    const pickerLabel = pickerSelector.querySelector('.lxt-picker-label');
