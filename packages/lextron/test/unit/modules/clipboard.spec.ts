...
import type { RegistryDefinition } from 'lextron-dom';
import {
  DirectionAttribute,
  DirectionClass,
  DirectionStyle,
} from 'lextron-formats/formats/direction.js';
import CodeBlock from 'lextron-formats/formats/code.js';
import { ColorClass, ColorStyle } from 'lextron-formats/formats/color.js';
...
describe('Clipboard', () => {
  describe('events', () => {
    const createLextron = () => {
      ...
      const editor = new Lextron(container, { registry });
    ...
    describe('paste', () => {
      ...
      test('pastes html data', async () => {
        ...
        expect(editor.getSelection()).toEqual(new Range(3));
      });
  ...
  describe('convert', () => {
    ...
    test('text with adjacent spaces', () => {
      ...
      expect(delta).toEqual(new ChangeSet().insert('simple  text'));
    });

    test('text with newlines', () => {
      const delta = createClipboard().convert({ text: 'simple\ntext' });
      expect(delta).toEqual(new ChangeSet().insert('simple\ntext'));
    });

    test('only text in html', () => {
      const delta = createClipboard().convert({ html: 'simple plain text' });
      expect(delta).toEqual(new ChangeSet().insert('simple plain text'));
    });
    ...
    test('whitespace', () => {
      ...
      expect(delta).toEqual(new ChangeSet().insert('0\n1 2  3  4\n5 6  7 8'));
    });

    test('multiple whitespaces', () => {
      const html = '<div>1   2    3</div>';
      const delta = createClipboard().convert({ html });
      expect(delta).toEqual(new ChangeSet().insert('1 2 3'));
    });
    ...
    test('inline whitespace', () => {
      ...
      expect(delta).toEqual(
        new ChangeSet().insert('0 ').insert('1', { bold: true }).insert(' 2'),
      );
    });
    ...
    test('newlines between inline elements', () => {
      ...
      const delta = createClipboard().convert({ html });
      expect(delta).toEqual(new ChangeSet().insert('foo bar'));
    });
    ...
    test('newlines between block elements', () => {
      const html = '<p>foo</p>\n<p>bar</p>';
      const delta = createClipboard().convert({ html });
      expect(delta).toEqual(new ChangeSet().insert('foo\nbar'));
    });
    ...
    test('space between empty paragraphs', () => {
      const html = '<p></p> <p></p>';
      const delta = createClipboard().convert({ html });
      expect(delta).toEqual(new ChangeSet().insert('\n'));
    });
    ...
    test('mixed inline and block', () => {
      ...
      expect(delta).toEqual(new ChangeSet().insert('One\nTwo'));
    });
    ...
    test('nested list', () => {
      const delta = createClipboard().convert({
        html: '<ol><li>One</li><li class="lxt-indent-1">Alpha</li></ol>',
      });
      expect(delta).toEqual(
        new ChangeSet()
          .insert('One\n', { list: 'ordered' })
    ...
    test('nested styles', () => {
      ...
      attributors.forEach((attributor) => {
        expect(createClipboard([attributor]).convert({ html })).toEqual(
          new ChangeSet().insert('Test', { color: 'blue' }),
        );
      });
    ...
    test('custom matcher', () => {
      ...
      const delta = clipboard.convert({
        html: 'http://github.com https://iamreetesh.com',
      });
      const expected = new ChangeSet()
        .insert('http://github.com', { link: 'http://github.com' })
        .insert(' ')
        .insert('https://iamreetesh.com', { link: 'https://iamreetesh.com' });
      expect(delta).toEqual(expected);
    });
    ...
    test('ignore empty elements except paragraphs', () => {
      ...
      const delta = createClipboard().convert({ html });
      expect(delta).toEqual(new ChangeSet().insert('hello\nmy\n\nworld'));
    });
  });
});
