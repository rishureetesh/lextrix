...
test('compose an epic', async ({ page, editorPage }) => {
  ...
  await page.keyboard.press('Enter');
  await page.type('.lxt-editor', CHAPTER);
  await page.keyboard.press('Enter');
  expect(await editorPage.root.innerHTML()).toEqual(
    [
      '<p>The Whale</p>',
  ...
  await page.click('.lxt-toolbar .lxt-bold');
  await page.click('.lxt-toolbar .lxt-italic');
  expect(await editorPage.root.innerHTML()).toEqual(
    [
      '<p><strong><em><span class="lxt-cursor">\uFEFF</span></em></strong></p>',
      `<p>${CHAPTER}</p>`,
      '<p><br></p>',
      `<p>\t${P1}</p>`,
      '<p><br></p>',
  ...
  let bold = await page.$('.lxt-toolbar .lxt-bold.lxt-active');
  let italic = await page.$('.lxt-toolbar .lxt-italic.lxt-active');
  ...
  bold = await page.$('.lxt-toolbar .lxt-bold.lxt-active');
  italic = await page.$('.lxt-toolbar .lxt-italic.lxt-active');
  expect(bold).not.toBe(null);
  expect(italic).not.toBe(null);
  ...
  await page.click('.lxt-toolbar .lxt-header[value="1"]');
  expect(await editorPage.root.innerHTML()).toEqual(
    [
  ...
  const header = await page.$('.lxt-toolbar .lxt-header.lxt-active[value="1"]');
