...
const updateSelectionDef = [
  ...
  return new Promise((resolve) => {
    document.addEventListener('selectionchange', () => {
      setTimeout(() => {
        resolve()
      }, 1); // wait for Lextron to update the internal selection
    }, {
      once: true,
    });
...
export default class EditorPage {
  ...
  get root() {
    return this.page.locator('.lxt-editor');
  }

  async open() {
    await this.page.goto('/');
    await this.page.waitForSelector('.lxt-editor', { timeout: 10000 });
  }

  async html(content: string, title = '') {
    await this.page.evaluate((html) => {
      // @ts-expect-error
      const contents = window.lextron.clipboard.convert({ html, text: '\n' });
      // @ts-expect-error
      return window.lextron.setContents(contents);
    }, `<p>${title}</p>${content}`);
  }
  ...
  getSelection() {
    return this.page.evaluate(() => {
      // @ts-expect-error
      return window.lextron.getSelection();
    });
  }
  ...
  async setSelection(
    ...
    await this.page.evaluate(
      // @ts-expect-error
      (range) => window.lextron.setSelection(range),
  ...
  async updateContents(delta: Op[], source: 'api' | 'user' = 'api') {
    await this.page.evaluate(
      ({ delta, source }) => {
        // @ts-expect-error
        window.lextron.updateContents(delta, source);
      },
      { delta, source },
    );
  }
  ...
  async setContents(delta: Op[]) {
    await this.page.evaluate((delta) => {
      // @ts-expect-error
      window.lextron.setContents(delta);
    }, delta);
  }
  ...
  getContents(): Promise<Op[]> {
    return this.page.evaluate(() => {
      // @ts-expect-error
      return window.lextron.getContents().ops;
    });
  }
  ...
  async moveCursorTo(query: string) {
    ...
    await this.page.evaluate(
      async ({ getTextNodeDef, updateSelectionDef, query, text }) => {
        ...
        const editor = window.document.querySelector('.lxt-editor');
