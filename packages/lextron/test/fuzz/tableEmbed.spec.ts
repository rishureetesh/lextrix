import type { ChangeAttributes } from 'lextron-change';
import ChangeSet from 'lextron-change';
import TableEmbed from 'lextron-modules/modules/tableEmbed.js';
import type {
  CellData,
  TableData,
  TableRowColumnOp,
} from 'lextron-modules/modules/tableEmbed.js';
...
const getRandomChange = (base: ChangeSet) => {
  const table: TableData = {};
  const dimension = {
    rows: new ChangeSet(
      (base.ops[0].insert as any)['table-embed'].rows || [],
    ).length(),
    columns: new ChangeSet(
  ...
  return new ChangeSet([attachAttributes({ retain: { 'table-embed': table } })]);
};
...
const getRandomBase = () => {
  ...
  return new ChangeSet([{ insert: { 'table-embed': table } }]);
};
