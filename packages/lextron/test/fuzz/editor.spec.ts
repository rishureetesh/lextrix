...
import Lextron from '../../src/lextron.js';
...
const isLineFinished = (delta: ChangeSet) => {
...
const generateSingleInsertDelta = (): ChangeSet['ops'][number] & {
  ...
  const op: ChangeOp & { insert: SingleInsertValue } = { insert };
...
const safePushInsert = (delta: ChangeSet, isDoc: boolean) => {
...
const generateChange = (
  ...
  const skipLength = allowedActions.includes('retain')
    ? randomInt(docLength)
    : 0;
  let change = new ChangeSet().retain(skipLength);
    ...
    case 'retain': {
      ...
      if (typeof nextOp.insert === 'string') {
        ...
        if (nextOp.insert.includes('\n')) {
          change.retain(
            retainLength,
            ChangeAttributes.diff(nextOp.attributes, generateAttributes('newline')),
          );
        } else {
          change.retain(
            retainLength,
            ChangeAttributes.diff(nextOp.attributes, generateAttributes('text')),
          );
        }
        break;
      }
...
describe('editor', () => {
  test('setContents()', () => {
    runFuzz(() => {
      const editor = new Lextron(document.createElement('div'));
      const delta = generateDocument();

      expect(delta.concat(new ChangeSet().delete(1))).toEqual(
  ...
  test('updateContents()', () => {
    runFuzz(() => {
      ...
      for (let i = 0; i < 800; i += 1) {
        const doc = editor.getContents();
        const change = generateChange(doc, randomInt(4) + 1);
        const diff = editor.updateContents(change);
        expect(change).toEqual(diff);
      }
    });
  });

  test('insertContents() vs applyChangeSet()', () => {
    const editor1 = new Lextron(document.createElement('div'));
    const editor2 = new Lextron(document.createElement('div'));
    ...
    runFuzz(() => {
      ...
      editor1.editor.insertContents(retain, change);
      editor2.editor.applyChangeSet(new ChangeSet().retain(retain).concat(change));

      const contents1 = editor1.getContents().ops;
      const contents2 = editor2.getContents().ops;
