...
const updateSelectionDef = [
  ...
  return new Promise((resolve) => {
    document.addEventListener('selectionchange', () => {
      setTimeout(() => {
        resolve()
      }, 1); // wait for Lextrix to update the internal selection
    }, {
      once: true,
    });
...
export default class EditorPage {
  ...
  async open() {
    ...
    await this.page.waitForSelector('.lxr-editor', { timeout: 10000 });
  }
  ...
  async moveCursorTo(query: string) {
    ...
    await this.page.evaluate(
      async ({ getTextNodeDef, updateSelectionDef, query, text }) => {
        ...
        const editor = window.document.querySelector('.lxr-editor');
