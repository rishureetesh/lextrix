...
import { LeafBlot, Registry } from 'lextrix-dom';
import {
  afterEach,
  beforeEach,
  describe,
...
import Toolbar from 'lextrix-modules/modules/toolbar.js';
import Lextrix, {
  expandConfig,
  globalRegistry,
  overload,
} from 'lextrix-core/core/lextrix.js';
...
describe('Lextrix', () => {
  test('imports', () => {
    Object.keys(Lextrix.imports).forEach((path) => {
      expect(Lextrix.import(path)).toBeTruthy();
    });
  });

  describe('register', () => {
    const imports = { ...Lextrix.imports };
    afterEach(() => {
      Lextrix.imports = imports;
    });
    ...
    test('register(path, target)', () => {
      ...
      expect(Lextrix.imports).toHaveProperty(lxrPath.module('counter'), Counter);
      expect(Lextrix.import(lxrPath.module('counter'))).toEqual(Counter);
    });

    test('register(formats)', () => {
      class MyCounterBlot extends LeafBlot {
        static blotName = 'my-counter';
        static className = 'lxr-my-counter';
      }
      Lextrix.register(MyCounterBlot);

      expect(Lextrix.imports).toHaveProperty(lxrPath.format('my-counter'), MyCounterBlot);
      expect(Lextrix.import(lxrPath.format('my-counter'))).toEqual(MyCounterBlot);
    });

    test('register(targets)', () => {
      class ABlot extends LeafBlot {
        static blotName = 'a-blot';
        static className = 'lxr-a-blot';
      }
      class AModule {}
      Lextrix.register({
        [lxrPath.format('a-blot')]: ABlot,
        [lxrPath.module('a-module')]: AModule,
      });

      expect(Lextrix.import(lxrPath.format('a-blot'))).toEqual(ABlot);
      expect(Lextrix.import(lxrPath.module('a-module'))).toEqual(AModule);
    });

    test('rejects legacy bare import paths', () => {
      expect(() => Lextrix.register('modules/legacy', class {})).toThrow(
        /Legacy import path/,
      );
      expect(() => Lextrix.import('delta')).toThrow(/Legacy import key/);
      expect(() => Lextrix.import('parchment')).toThrow(/Legacy import key/);
    });
  });

  describe('construction', () => {
    test('empty', () => {
      const editor = new Lextrix(createContainer());
      expect(editor.getContents()).toEqual(new ChangeSet().insert('\n'));
      expect(editor.root.innerHTML).toMatchInlineSnapshot('"<p><br></p>"');
    });

    test('text', () => {
      const editor = new Lextrix(createContainer('0123'));
      expect(editor.getContents()).toEqual(new ChangeSet().insert('0123\n'));
      expect(editor.root.innerHTML).toMatchInlineSnapshot('"<p>0123</p>"');
    });

    test('newlines', () => {
      const editor = new Lextrix(
    ...
    test('formatted ending', () => {
      const editor = new Lextrix(
        createContainer('<p class="lxr-align-center">Test</p>'),
      );
      expect(editor.getContents()).toEqual(
        new ChangeSet().insert('Test').insert('\n', { align: 'center' }),
      );
      expect(editor.root.innerHTML).toMatchInlineSnapshot(
        '"<p class="lxr-align-center">Test</p>"',
      );
    });
  });
  ...
  describe('api', () => {
    const setup = () => {
      const editor = new Lextrix(createContainer('<p>0123<em>45</em>67</p>'));
  ...
  describe('events', () => {
    const setup = () => {
      const editor = new Lextrix(createContainer('<p>0123</p>'));
      editor.update();
      vitest.spyOn(editor.emitter, 'emit');
      const oldChangeSet = editor.getContents();
      return { editor, oldChangeSet };
  ...
  describe('setContents()', () => {
    ...
    test('no trailing newline', () => {
      const editor = new Lextrix(createContainer('<h1>Welcome</h1>'));
    ...
    test('block embed', () => {
      const editor = new Lextrix(createContainer('<p>Hello World!</p>'));
  ...
  describe('setText()', () => {
    ...
    test('return carriage', () => {
      const editor = new Lextrix(createContainer('<p>Test</p>'));
      editor.setText('\r');
      expect(editor.root.innerHTML).toMatchInlineSnapshot('"<p><br></p>"');
    });
  ...
  describe('expandConfig', () => {
    ...
    test('lextrix < module < theme < user', () => {
  ...
  describe('overload', () => {
    test('(index:number, length:number)', () => {
      ...
      expect(source).toBe(Lextrix.sources.API);
    });
    ...
    test('(index:number, length:number, format:string, value:boolean, source:string)', () => {
      ...
      expect(source).toBe(Lextrix.sources.USER);
    });
    ...
    test('(index:number, length:number, format:string, value:string, source:string)', () => {
      ...
      expect(index).toBe(0);
      expect(length).toBe(1);
      expect(formats).toEqual({ color: Lextrix.sources.USER });
      expect(source).toBe(Lextrix.sources.USER);
    });
    ...
    test('(index:number, source:string)', () => {
      const [index, length, formats, source] = overload(0, Lextrix.sources.USER);
      expect(index).toBe(0);
    ...
    test('(range:range, format:string, value:string, source:string)', () => {
      ...
      expect(index).toBe(0);
      expect(length).toBe(1);
      expect(formats).toEqual({ color: Lextrix.sources.API });
      expect(source).toBe(Lextrix.sources.USER);
    });
  ...
  describe('placeholder', () => {
    ...
    test('blank editor', () => {
      ...
      expect([...editor.root.classList]).toContain('lxr-blank');
    });

    test('with text', () => {
      const { editor } = setup();
      editor.setText('test');
      expect([...editor.root.classList]).not.toContain('lxr-blank');
    });
  ...
  describe('scrollSelectionIntoView', () => {
    ...
    test('scroll upward', async () => {
      ...
      const editor = new Lextrix(editorContainer);
