...
import { BaseTooltip } from 'lextrix-themes';
...
describe('BaseTooltip', () => {
  const setup = () => {
    ...
    const editor = new Lextrix(container, { registry: createRegistry([Video]) });
    const tooltip = new Tooltip(editor);
  ...
  describe('save', () => {
    test('converts youtube video url to embedded', () => {
      ...
      expect(
        (container.querySelector('.lxr-video') as HTMLVideoElement).src,
