...
import Table from 'lextrix-modules/modules/table.js';

const createLextrix = (html: string) => {
  Lextrix.register({ [lxrPath.module('table')]: Table }, true);
...
describe('Table Module', () => {
  describe('insert table', () => {
    test('empty', () => {
      const editor = createLextrix('<p><br></p>');
    ...
    test('split', () => {
      const editor = createLextrix('<p>0123</p>');
      const table = editor.getModule('table') as Table;
  ...
  describe('modify table', () => {
    const setup = () => {
      ...
      `;
      const editor = createLextrix(tableHTML);
      const table = editor.getModule('table') as Table;
      return { editor, table };
    };
