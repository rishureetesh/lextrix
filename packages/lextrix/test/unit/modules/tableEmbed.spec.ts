...
import { tableHandler } from 'lextrix-modules/modules/tableEmbed.js';
...
describe('tableHandler', () => {
  ...
  afterEach(() => {
    ChangeSet.unregisterEmbed('table-embed');
  });

  describe('compose', () => {
    test('adds a row', () => {
      const base = new ChangeSet([
        {
          insert: {
            'table-embed': {
              rows: [
    ...
    test('removes a cell attributes', () => {
      ...
      expect(base.compose(change)).toEqual(
        new ChangeSet([{ insert: { 'table-embed': {} } }]),
      );
    });
  ...
  describe('transform', () => {
    ...
    test('transform cell attributes', () => {
      ...
      expect(change1.transform(change2, true)).toEqual(
        new ChangeSet([{ retain: { 'table-embed': {} } }]),
      );
    });
  });
