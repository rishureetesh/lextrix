...
class SyntaxCodeBlockContainer extends CodeBlockContainer {
  ...
  highlight(
    highlight: (text: string, language: string) => ChangeSet,
    forced = false,
  ) {
    ...
    if (forced || this.forceNext || this.cachedText !== text) {
      if (text.trim().length > 0 || this.cachedText == null) {
        const oldChangeSet = this.children.reduce((delta, child) => {
          // @ts-expect-error
          return delta.concat(blotToChangeSet(child, false));
        }, new ChangeSet());
        const delta = highlight(text, language);
        oldChangeSet.diff(delta).reduce((index, { retain, attributes }) => {
        ...
        (node, delta) => {
          ...
          if (value) {
            return delta.compose(
              new ChangeSet().retain(delta.length(), {
