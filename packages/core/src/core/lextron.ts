...
import * as Dom from 'lextron-dom';
import type { ChangeOp } from 'lextron-change';
...
const debug = logger('lextron');

const globalRegistry = new Dom.Registry();
Dom.ParentBlot.uiClass = 'lxt-ui';

/**
 * Options for initializing a Lextron instance
 */
export interface LextronOptions {
  theme?: string;
...
/**
 * Similar to LextronOptions, but with all properties expanded to their default values,
 * and all selectors resolved to HTMLElements.
 */
export interface ExpandedLextronOptions
  extends Omit<LextronOptions, 'theme' | 'formats'> {
...
class Lextron {
  static DEFAULTS = {
    ...
    theme: 'default',
  } satisfies Partial<LextronOptions>;
  static events = Emitter.events;
  static sources = Emitter.sources;
  static version = typeof LEXTRON_VERSION === 'undefined' ? 'dev' : LEXTRON_VERSION;

  static imports: Record<string, unknown> = {
    change: ChangeSet,
    dom: Dom,
    [lxtPath.core.module]: Module,
    [lxtPath.core.theme]: Theme,
  };
  ...
  static import(name: typeof lxtPath.core.module): typeof Module;
  static import(name: `lxt/themes/${string}`): typeof Theme;
  ...
  static register(...args: any[]): void {
    if (typeof args[0] !== 'string') {
      ...
      if (typeof name === 'string') {
        ...
        this.register(lxtPath.format(name), target, overwrite);
      } else {
  ...
  options: ExpandedLextronOptions;

  constructor(container: HTMLElement | string, options: LextronOptions = {}) {
    this.options = expandConfig(container, options);
    this.container = this.options.container;
    if (this.container == null) {
      debug.error('Invalid Lextron container', container);
      return;
    }
    if (this.options.debug) {
      Lextron.debug(this.options.debug);
    }
    const html = this.container.innerHTML.trim();
    this.container.classList.add('lxt-container');
    this.container.innerHTML = '';
    instances.set(this.container, this);
    this.root = this.addContainer('lxt-editor');
    this.root.classList.add('lxt-blank');
    this.emitter = new Emitter();
    const scrollBlotName = Dom.ScrollBlot.blotName;
    const ScrollBlot = this.options.registry.query(scrollBlotName);
    if (!ScrollBlot || !('blotName' in ScrollBlot)) {
      throw new Error(
        `Cannot initialize Lextron without "${scrollBlotName}" blot`,
      );
    }
    ...
    this.emitter.on(Emitter.events.EDITOR_CHANGE, (type) => {
      if (type === Emitter.events.TEXT_CHANGE) {
        this.root.classList.toggle('lxt-blank', this.editor.isBlank());
      }
    });
  ...
  enable(enabled = true) {
    ...
    this.container.classList.toggle('lxt-disabled', !enabled);
  }
  ...
  /**
   * @deprecated Use Lextron#scrollSelectionIntoView() instead.
   */
  scrollIntoView() {
    console.warn(
      'Lextron#scrollIntoView() has been deprecated and will be removed in the near future. Please use Lextron#scrollSelectionIntoView() instead.',
    );
    this.scrollSelectionIntoView();
  }
  ...
  setContents(
    ...
    return modify.call(
      ...
      () => {
        ...
        // Lextron will set empty editor to \n
  ...
  setSelection(
    ...
    if (index == null) {
      ...
      this.selection.setRange(null, length || Lextron.sources.API);
    } else {
      // @ts-expect-error
      [index, length, , source] = overload(index, length, source);
...
function omitUndefinedValuesFromOptions(obj: LextronOptions) {
...
function expandConfig(
  ...
  options: LextronOptions,
): ExpandedLextronOptions {
  const container = resolveSelector(containerOrSelector);
  if (!container) {
    throw new Error('Invalid Lextron container');
  }

  const themeName = options.theme;
  const shouldUseDefaultTheme =
    !themeName || themeName === Lextron.DEFAULTS.theme;
  const theme: ThemeConstructor = shouldUseDefaultTheme
    ? Theme
    : (Lextron.import(
        lxtPath.theme(themeName) as `lxt/themes/${string}`,
      ) as ThemeConstructor);
  if (!theme) {
    throw new Error(`Invalid theme ${options.theme}. Did you register it?`);
  }

  const { modules: lextronModuleDefaults, ...lextronDefaults } = Lextron.DEFAULTS;
  ...
  const modules: ExpandedLextronOptions['modules'] = merge(
    {},
    expandModuleConfig(lextronModuleDefaults),
  ...
  return {
    ...
    modules: Object.entries(modules).reduce(
      (modulesWithDefaults, [name, value]) => {
        ...
        const moduleClass = Lextron.import(lxtPath.module(name));
        if (moduleClass == null) {
          debug.error(
...
export { globalRegistry, expandConfig, overload, Lextron as default };
