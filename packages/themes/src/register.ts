import type Lextrix from 'lextrix-core';
import { lxrPath } from 'lextrix-core/registry-paths.js';

import BubbleTheme from './themes/bubble.js';
import DawnTheme from './themes/dawn.js';
import SlateTheme from './themes/slate.js';
import SnowTheme from './themes/snow.js';

export function registerThemes(editor: typeof Lextrix, overwrite = false) {
  editor.register(
    {
      [lxrPath.theme('bubble')]: BubbleTheme,
      [lxrPath.theme('dawn')]: DawnTheme,
      [lxrPath.theme('slate')]: SlateTheme,
      [lxrPath.theme('snow')]: SnowTheme,
    },
    overwrite,
  );
}

export { default as BaseTheme, BaseTooltip } from './themes/base.js';
export { BubbleTheme, DawnTheme, SlateTheme, SnowTheme };
