...
class DawnTooltip extends BaseTooltip {
  static TEMPLATE = [
...
class DawnTheme extends BaseTheme {
  constructor(lextrix: Lextrix, options: ThemeOptions) {
    ...
    this.lextrix.container.classList.add('lxr-dawn');
  }

  extendToolbar(toolbar: Toolbar) {
    if (toolbar.container != null) {
      toolbar.container.classList.add('lxr-dawn');
      this.buildButtons(toolbar.container.querySelectorAll('button'), icons);
      this.buildPickers(toolbar.container.querySelectorAll('select'), icons);
      // @ts-expect-error
      this.tooltip = new DawnTooltip(this.lextrix, this.options.bounds);
...
DawnTheme.DEFAULTS = merge({}, BaseTheme.DEFAULTS, {
  modules: {
    toolbar: {
      handlers: {
        link(value: string) {
