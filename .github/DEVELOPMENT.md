...
| `packages/lextrix-serialize` | HTML, Markdown, MDX, JSON ↔ ChangeSet |
...
```bash
npm run prepublish:check:dry-run
```
...
The build writes a trimmed `package.json` to `packages/lextrix/dist/dist/`.
